...
def game_board(request, room_id):
    ...
    current_position = request.session.get(f'position_{room_id}', 1)
    last_roll = request.session.get(f'last_roll_{room_id}')
    ...
    if request.method == 'POST' and request.POST.get('roll'):
        ...
        if 94 <= current_position <= 99:
            if new_position > 100:
                ...
                # Win condition
                request.session[f'position_{room_id}'] = 1
                request.session[f'last_roll_{room_id}'] = None
                return redirect(reverse('snake_ladder:game_board', args=[room_id]))
        elif new_position > 100:
            # Reset game for positions < 94
            new_position = 1
            request.session[f'position_{room_id}'] = new_position
            request.session[f'last_roll_{room_id}'] = None
            return redirect(reverse('snake_ladder:game_board', args=[room_id]))
        ...
        request.session[f'position_{room_id}'] = new_position
        request.session[f'last_roll_{room_id}'] = dice_roll
        return redirect(reverse('snake_ladder:game_board', args=[room_id]))
